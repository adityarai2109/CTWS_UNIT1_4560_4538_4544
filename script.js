...
async function start() {
	...
	// Prepare input.
	const EXPECTED_IMAGE_SIZE = 224;
	const img = document.querySelector("img");
	const inputTensor = tf.cast(
		tf.image
			.resizeBilinear(tf.browser.fromPixels(img), [
				EXPECTED_IMAGE_SIZE,
				EXPECTED_IMAGE_SIZE,
			]) // Resize
			.expandDims(),
	...
	const strResult = results
		.map((result) => LABELS[result.index])
		.join("<br>");
	document.querySelector(".result").innerHTML =
		"<h3>Predicted Output</h3><br/>" + strResult;
}
